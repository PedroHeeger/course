...
Agora foi criada uma instância do serviço **Amazon Relational Database Service (RDS)**, que é um banco de dados relacional, com o comando abaixo. Para listar todas as instâncias criadas nesse serviço foi utilizado `aws rds describe-db-instances`, que exibiu em formato JSON. Para um formato em texto, exibindo apenas os nomes das instâncias foi utilizado o comando `aws rds describe-db-instances --query "DBInstances[].DBInstanceIdentifier" --output text`. Ainda para filtrar o status da instância criada e ver se já está em execução foi utilizado o comando `aws rds describe-db-instances --db-instance-identifier banco --query 'DBInstances[0].DBInstanceStatus' --output text` que também exibiu em um formato de texto filtrando a informação desejada. 
...
Com a instância em execução, foi copiado o `endpoint` do banco de dados, que pode ser visualizado com o comando `aws rds describe-db-instances --query "DBInstances[].Endpoint[].Address" --output text`, e colocado lá na aplicação que estava rodando no navegador, na opção `Base de Dados`. Além dela, foram passadas as informações de credenciais de acesso ao banco de dados e o nome do banco de dados. Então, foi clicado em enviar e aplicação iniciou a execução direcionando para uma outra página. Nesse momento, estava sendo realizado uma vinculação da aplicação no serviço EC2 com o banco de dados do serviço RDS, cujas as credenciais foram passadas no navegador. Após o retorno para a página da aplicação, na opção `Base de Dados` foi exibida agora uma lista de contatos com duas linhas de dados inseridas, conforme imagem 06. Nessa lista foi possível cadastrar novos contatos por meio do link clicável `Adicionar Contatos` que enviava as informações preenchidas para o banco de dados, onde tudo era armazenado. Além disso, a aplicação extraía do banco de dados essas informações para a página, compondo a lista de contatos, sendo possível também remover ou editar um contato da lista, tudo isso pelo navegador. Infelizmente, a opção de editar não estava funcionando na aplicação.
...
A medida que as informações foram adicionadas o banco de dados foi alimentado. Para acessar esse banco de dados, foi utilizado a maquina virtual do EC2 em um acesso remoto no software **PuTTY** e com o comando `mysql -h banco.cbjhpxxvj7m2.us-east-1.rds.amazonaws.com -u admin -p` e a senha o usuário `admin` foi concedido o acesso. O parâmetro `-h` define o endpoint da instância criada no serviço RDS. A próxima imagen (07) mostra a tabela com os dados inseridos no navegador, através do acesso remoto a maquina virtual feito pelo **PuTTY**.
...
Para remover a instância do serviço RDS foi utilizado o comando `aws rds delete-db-instance --db-instance-identifier banco --skip-final-snapshot` que ignorou a execução de um snapshot do estado do banco, excluindo todos os dados dentro desse banco.

<a name="item02"><h4>Class 02</h4></a>[Back to summary](#item0)

Esta aula inicou com a criação de um bucket no serviço **Amazon S3**. Por padrão, o S3 mostrou os buckets para todas as regiões, mas foi possível escolher a região específica para construção do bucket, que neste caso foi a `us-east-1` (Virgínia do Norte), pois foi onde todos os serviços estavam sendo criados. Não é possível criar um bucket com mesmo nome, mesmo que seja de outra conta da AWS. Os nomes dos buckets são únicos, não podem ser iguais.

Com o comando `aws s3api create-bucket --bucket armazenamentos3 --region us-east-1` foi criado o bucket de nome `armazenamentos3` na região determinada. Porém, algumas configurações realizadas pelo professor do curso não foram executadas, portanto foi necessário realizar algumas alterações no bucket. A primeira delas foi com o comando `aws s3api put-public-access-block --bucket armazenamentos3 --public-access-block-configuration "BlockPublicAcls=false,IgnorePublicAcls=false,BlockPublicPolicy=false,RestrictPublicBuckets=false"` que desativava o bloqueio de acesso público as quatro opções existentes. 

A segunda alteração foi a habilitação da lista de controle de acesso (**Access Control List (ACL)**) que foi executado com o comando `aws s3api put-bucket-acl --bucket armazenamentos3 --acl bucket-owner-full-control` tornando público as opções de leitura e gravação apenas para o proprietário do bucket. Contudo, esse comando só pode ser realizado se a regra propriedade do objeto (`ObjectOwnership`) criada em controles de propriedade (`OwnershipControls`) do bucket, estivesse definida como proprietário do bucket preferencial/proprietário do bucket preferido (`Bucket Owner Preferred`). Como no bucket desenvolvido não foi especificada essa regra, o padrão foi utilizado que é proprietário do bucket aplicado/imposto pelo proprietário do bucket (`Bucket Owner Enforced`). Para verificar realmente qual a definição dessa regra, o comando `aws s3api get-bucket-ownership-controls --bucket armazenamentos3` mostrou todas as regras elaboradas no controles de propriedade do bucket. Para alterar a regra e permitir a alteração do ACL, foi utilizado o comando `aws s3api put-bucket-ownership-controls --bucket armazenamentos3 --ownership-controls="Rules=[{ObjectOwnership=BucketOwnerPreferred}]"`.

A solução para essa situação foi descoberta através de pesquisas, no qual uma explição foi encontrada no seguinte [site](https://stackoverflow.com/questions/76097031/aws-s3-bucket-cannot-have-acls-set-with-objectownerships-bucketownerenforced-s) do **Stack Overflow**. Onde foi relatado que desde 25/04/2023, a Amazon alterou as configurações padrão para buckets recém-criados. A ACL em buckets foi considerada uma prática errada e para desencorajar seu uso a opção `BucketOwnerEnforced` passou a ser a padrão. A opção `BucketOwnerEnforced` quer dizer que as listas de controle de acesso (ACLs) estão desabilitadas e não afetam mais as permissões. O proprietário do bucket possui automaticamente e tem controle total sobre cada objeto no bucket. O bucket aceita apenas solicitações PUT que não especificam uma ACL ou ACLs de controle total do proprietário do bucket, como a ACL pronta de controle total do proprietário do bucket ou uma forma equivalente dessa ACL expressa no formato XML. Por causa disso, foi alterado a regra para opção `BucketOwnerPreferred`.

Analisando o comando `create-bucket` através de `aws s3api create-bucket help`, foi possível verificar um parâmetro de nome `--object-ownership`, onde nele pode ser definido as três seguintes condições;
...
Logo, foi entendido que podia criar o bucket já configurando essa regra e definindo a ACL como habilitada. O comando a seguir criou o bucket corretamente igual a execução do professor do curso. Após isso, deve desativar as quatro opções do bloqueio de acesso público. Para a listagem dos buckets existentes, existe o `aws s3 ls` ou  o `aws s3api list-buckets`, que lista todos os buckets em todas as regiões, e o `aws s3 ls --region us-east-1` ou `aws s3api list-buckets --region us-east-1`, que lista apenas os buckets da região especificada. Os comandos `aws s3api` exibem em formato JSON como os demais comandos utilizados, enquanto o comando `aws s3` tem uma abordagem diferente, exibindo em formato de texto, mas também é possível passar o parâmetro `--output text` no outro tipo que será exibido em formato de texto.
...
A próxima etapa foi a ativação de hospedagem de site estático do bucket S3. Para isso foi utilizado o comando `aws s3 website s3://armazenamentos3 --index-document index.html --error-document index.html`, onde dois arquivos **HTML** foram definidos, um para o site e outro para casos de erro. Ao tentar acessar o site com o endpoint no navegador foi ocorrido um erro, pois ainda era necessário criar uma política de segurança para liberar o acesso a esse bucket. Essa política foi elaborada no arquivo JSON [bucketPolicy.JSON](resources/bucket-policy.json), onde foi definido os parâmetros: `Effect` como `Allow`, ou seja, permitir; `Principal` como `*`, ou seja, indica que são todos os objetos do bucket; `Actions` como `s3:GetObject`, para retornar os objetos; e o `Amazon Resource Name (ARN) / Resource` definido como o mesmo do bucket acrescido de `/*` (`arn:aws:s3:::armazenamentos3/*`). Para criar essa política para o bucket foi utilizado o comando abaixo.
...
Para exibir as configurações de hospedagem do site estático e as polices criadas para esse bucket foram utilizados, respectivamente, os comandos 
...
Uma observação importante foi que a imagem só poderia ser criada após construção da primeira instância, acesso a aplicação via navegador, preenchimento e envio das informações dos campos na opção **Banco de Dados**. Isso porque, uma vez que a vinculação da aplicação que está em execução no serviço EC2 com o serviço RDS de banco de dados for realizada, não é possível trocar essa vinculação. Se essa vinculação for feita após a criação da imagem, vai funcionar para uma instância, mas para a segunda não vai, pois os dados inseridos nos campos já foram utilizados na primeira instância. Essa situação é algo que poderia ter sido tratada nos arquivos de código da aplicação.
...
Para verificar se o grupo de destino foi criado utilizou o comando `aws elbv2 describe-target-groups` que lista todos os grupos existentes, e o `aws elbv2 describe-target-groups --query "TargetGroups[].TargetGroupName" --output text` que listou apenas o nome do grupo de destino em formato de texto. Para listar o ARN dos grupos de destino em formato de texto foi utilizado comando `aws elbv2 describe-target-groups --query "TargetGroups[].TargetGroupArn" --output text`. A vinculação das instâncias ao grupo de destino foi realizada com o comando abaixo, que utilizou o ARN do grupo de destino e os ids das instâncias construídas no EC2. Foi verificado se as instâncias foram adicionadas ao grupo com o comando `aws elbv2 describe-target-health --target-group-arn arn:aws:elasticloadbalancing:us-east-1:005354053245:targetgroup/Gcurso080/2f5b71e03dee2b8a`.
...
Com relação ao Load Balancer, para listar os existentes foi utilizado o comando `aws elbv2 describe-load-balancers` que mostra em formato JSON. Para filtrar apenas os nomes dos Loads Balancers criados em formato de texto foi o comando `aws elbv2 describe-load-balancers --query "LoadBalancers[].LoadBalancerName" --output text` e para filtrar o ARN dele foi o `aws elbv2 describe-load-balancers --query "LoadBalancers[].LoadBalancerArn" --output text`. Em seguida, foi criado um Listener HTTP conforme comando abaixo, que houvia o tráfego na porta 80 e encaminhava para o grupo de destino criado, onde estavam as instâncias EC2, para que elas recebessem as requisições balanceadamente.
