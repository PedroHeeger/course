...
- A pasta `secrets` era onde estavam os dados pessoais sensíveis e por isso não foi versionada para o **GitHub**. Ela foi ramificada nos seguintes diretórios:
    - A sub-pasta `.aws` armazenando os arquivos de credenciais e configuração do usuário do IAM `PedroHeegerAdmin` da minha conta da **AWS** para utilizar na **AWS CLI** da instância.
    - A sub-pasta `.docker` que armazenava o arquivo configuração com a autenticação no usuário da minha conta no **Docker Hub**
    - A sub-pasta `keyPair` contendo arquivos par de chaves `.pem` gerados para realização de acesso remoto da maquina física **Windows** na instância do EC2.
...
Etapas do arquivo `udFileCurso116.sh`:
  ...
  - ETAPA 2: BAIXANDO OS ARQUIVOS DO PROJETO DO GITHUB: Na etapa dois, a pasta do repositório do **GitHub** do professor do curso foi baixada e sub-pastas necessárias para o projeto do curso foram criadas.
  ...
  - ETAPA 4: DOCKER COMPOSE PROJETO (AULA 2): Na qurta etapa, foi realizado o build de uma imagem **Docker** exatamente igual a aplicação web anterior executada direto na instância, no qual essa imagem foi enviada para um repositório do **Docker Hub** e então, utilizado o arquivo de manifesto **YAML** do **Docker Compose**, foi implantando a aplicação web, agora containerizada, e também o container de banco de dados para persistência dos dados da aplicação.
  - ETAPA 5: KUBERNETES PROJETO (AULA 3): Na última etapa deste arquivo em **Bash** na instância do EC2 da **AWS** foi realizado o projeto de número 3 que era o projeto principal, utilizando os dois softwares **Docker** e **Kubernetes**. A partir daí, a instância pôde ser encerrada, concluindo as aulas 1 e 2 (projetos 1, 2 e 3) e retornado para o arquivo de script em **PowerShell** na maquina física para seguir para etapa 2 dele.
